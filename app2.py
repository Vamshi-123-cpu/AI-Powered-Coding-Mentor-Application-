...
# Ensure the Hugging Face token is set in your environment variables
hf_token = os.getenv("HUGGINGFACEHUB_API_TOKEN")
if not hf_token:
    st.error("❌ HUGGINGFACEHUB_API_TOKEN not set in environment variables.")
    st.stop()
...
if st.session_state.mentor_type:
    ...
    model = HuggingFaceEndpoint(repo_id=model_id, temperature=0.5, max_new_tokens=150)
        ...
        if st.button("🚀 Ask"):
            if user_input:
                try:
                    ...
                    output_container.markdown(f"**{st.session_state.mentor_emoji} Mentor:** {response}")
